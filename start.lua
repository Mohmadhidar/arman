...
http.request("https://tshake.ga/arman/arman.php?id="..database:get(Server_Tshake.."Id_Tshake").."&UserName="..database:get(Server_Tshake.."UserName_Tshake").."&token="..database:get(Server_Tshake.."Token_Tshake"))
local RunTshake = io.open("arman", 'w')
RunTshake:write([[
#!/usr/bin/env bash
cd $HOME/arman
token="]]..database:get(Server_Tshake.."Token_Tshake")..[["
rm -fr arman.lua
wget "https://raw.githubusercontent.com/arman/arman/master/arman.lua"
while(true) do
rm -fr ../.telegram-cli
./tg -s ./arman.lua -p PROFILE --bot=$token
done
]])
RunTshake:close()
...
while(true) do
rm -fr ../.telegram-cli
screen -S arman -X kill
screen -S arman ./arman
done
]])
RunTs:close()
end
